<?php
	...
	if($_SERVER["REQUEST_METHOD"]=="POST")
	{
		if(check_admin_referer( 'edit-menu_', 'Chintan_Web_PSlider_Nonce' ))
		{
			...
			if(isset($_POST['chintan_webSlSave']))
			{
				...
				for($i=1;$i<=$JumboHidNum;$i++)
				{
					$wpdb->query($wpdb->prepare("INSERT INTO $table_name1 (id, SL_Img_Title, Sl_Img_Description, Sl_Img_Url, Sl_Link_Url, Sl_Link_NewTab, Sl_Number) VALUES (%d, %s, %s, %s, %s, %s, %d)", '', $SL_Img_Title[$i], $Sl_Img_Description[$i], $Sl_Img_Url[$i], $Sl_Link_Url[$i], $Sl_Link_NewTab[$i], $Chintan_Web_Sl_Id));
				}

				for($i=1;$i<=$JumboHidNum;$i++)
				{
					$wpdb->query($wpdb->prepare("INSERT INTO $table_name1_1 (id, Sl_Video_Url, Sl_Number) VALUES (%d, %s, %d)", '', $Sl_Video_Url[$i], $Chintan_Web_Sl_Id));
				}
			}
	...
	$Chintan_WebDat = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE id>%d", 0));
	$Chintan_WebIm = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name1 WHERE id>%d order by id", 0));
	$Chintan_WebVideo = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name1_1 WHERE id>%d order by id", 0));
	$Chintan_WebEff = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name2 WHERE id>%d", 0));
...
<form method="POST" enctype="multipart/form-data">
	...
	<div style="position: relative; width: 100%; right: 1%; height: 50px;">
		...
		<input type='submit' class='JSaveSlider' value='Save' name='chintan_webSlSave' />
		<input type='submit' class='JUpdateSlider' value='Update' name='chintan_webSlUpdate' />
	...
	<div class='Table_Data_chintan_web_Content' >
		<div class='Table_Data_chintan_web1'>
			<table class='chintan_web_Tit_Table'>
				<tr class='chintan_web_Tit_Table_Tr'>
					<td>No</td>
					<td>Slider Name</td>
					<td>Slider Option</td>
					<td>Images</td>
			...
			<table class='chintan_web_Tit_Table2'>
			<?php for($i=0;$i<count($Chintan_WebDat);$i++){?> 
				<tr class='chintan_web_Tit_Table_Tr2'>
					<td><?php echo $i+1; ?></td>
					<td><?php echo $Chintan_WebDat[$i]->Slider_Title; ?></td>
					<td><?php echo $Chintan_WebDat[$i]->Slider_Type; ?></td>
					<td><?php echo $Chintan_WebDat[$i]->Slider_IMGS_Quantity; ?></td>
					<td onclick="chintan_web_Copy_Sl(<?php echo $Chintan_WebDat[$i]->id;?>)"><i class='jIcFileso chintan_web chintan_web-files-o'></i></td>
					<td onclick="chintan_web_Edit_Sl(<?php echo $Chintan_WebDat[$i]->id;?>)"><i class='jIcPencil chintan_web chintan_web-pencil'></i></td>
					<td onclick="chintan_web_Delete_Sl(<?php echo $Chintan_WebDat[$i]->id;?>)"><i class='jIcDel chintan_web chintan_web-trash'></i></td>
				</tr>
			<?php } ?>
			</table>
		</div>
		<div class='Table_Data_chintan_web2'>
			<table class="chintan_web_ShortTable" style="display: table; float: right;">
				...
				<tr>
					<td class="chintan_web_Slider_ID">[Chintan_Web_Slider id="1"]</td>
				</tr>
				<tr>
				...
				<tr>
					<td class="JMBSL">&lt;?php echo do_shortcode('[Chintan_Web_Slider id="1"]');?&gt;</td>
				</tr>
			</table>
			<table class='chintan_web_SaveSl_Table' style='float:left;'>
				<tr>
				...
				<tr>
					<td colspan='2'>
						<select class='JSlInput JSType' name='Slider_Type'>
							<?php for($i=0;$i<count($Chintan_WebEff);$i++){ ?>
								<option value='<?php echo $Chintan_WebEff[$i]->slider_name;?>'><?php echo $Chintan_WebEff[$i]->slider_name;?></option>
							<?php }?>
						</select>
					</td>
				</tr>
				...
				<tr>
					<td style='width:100%' colspan='2'>
						<div id="wp-content-media-buttons" class="wp-media-buttons">
							<a href="#" class="button insert-media add_media" style="border:1px solid rgba(0, 73, 107, 0.8); color:rgba(0, 73, 107, 0.8); background-color:#f4f4f4" data-editor="chintan_web_videoSrc_1" title="Add Image" id="chintan_webVideoSrc" onclick="chintan_web_Video_Src_Clicked()">
								<span class="wp-media-buttons-icon"></span>Add Video
							</a>
						</div>
						<input type="text" style="display:none;" id="chintan_web_videoSrc_1" class='JSlInputVideo JSlInputVideo2'>
						<input type="text" style="display:none;" id="Chintan_Web_SlVid_Vid_1" class='JSlInputVideo JSlInputVideo2'>
					</td>
				</tr>
				<tr>
					<td colspan='2'>
						<input type="text" id="chintan_web_videoSrc_2" class="JSlInputVideo JSlInputVideo2 chintan_web_ImgSrc_Select" readonly>
					</td>
				</tr>
				<tr>
					<td style='width:100%' colspan='2'>
						<div id="wp-content-media-buttons" class="wp-media-buttons">
							<a href="#" class="button insert-media add_media" style="border:1px solid rgba(0, 73, 107, 0.8); color:rgba(0, 73, 107, 0.8); background-color:#f4f4f4" data-editor="chintan_web_imgSrc_1" title="Add Image" id="chintan_webImgSrc" onclick="chintan_web_Img_Src_Clicked()">
								<span class="wp-media-buttons-icon"></span>Add Image
							</a>
						</div>
						<input type="text" style="display:none;" id="chintan_web_imgSrc_1" class='JSlInput JSlInput2'>
					</td>
				</tr>
				<tr>
					<td colspan='2'>
						<input type="text" id="chintan_web_imgSrc_2" class="JSlInput JSlInput2 chintan_web_ImgSrc_Select" readonly>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td class='addImJ' colspan='3' style='padding:0px;'>
						<input type='button' class='JSVBut' onclick="chintan_web_Save()" value='Save' />
						<input type='button' class='JUPBut' onclick="chintan_web_Update()" value='Update' />
						<input type='button' class='JRBut' onclick="chintan_web_Res()" value='Reset'/>
			...
			<table class='chintan_web_SaveSl_Table2'>
				<tr>
					<td> No </td>
					<td> Image </td>
					<td> Title </td>
			...
			<table class='chintan_web_SaveSl_Table3' onmousemove='chintan_webSortable()'></table>
		</div>
	</div>
</form>